...
#endif
{
    ...
    rateRedux->defaultValue = 100;
    rateRedux->currentValue = 100;
    rateRedux->name = "Rate";
    addParameter(rateRedux);

    bitRedux = new BitCrush_Parameter();
    bitRedux->defaultValue = 1000;
    bitRedux->currentValue = 1000;
    bitRedux->name = "Bits";
...
void BitCrusherAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int i = 0; i < numSamples; i++) 
        {
            if (i % rate != 0)
            {
                holder = (int)(channelData[i] * mult);
                holder_f = (float)(holder / mult);
                channelData[i]= holder_f;
            }
            else
            {
                channelData[i] = 0;
